...
{
    public class PdkParcelShopDeliveryProvider : Weight.WeightDeliveryProvider
    {
        /// <summary>
        ...
        {
            return (IDeliveryTypeSetupControl)BuildManager.CreateInstanceFromVirtualPath("~/Admin/Catalog/Delivery/Pdk/PdkParcelShopSetupControl.ascx", typeof(Control));
        }
        ...
        {
            return (IDeliveryTypeCheckoutControl)BuildManager.CreateInstanceFromVirtualPath("~/Admin/Catalog/Delivery/Pdk/PdkParcelShopCheckoutControl.ascx", typeof(Control));
        }
        ...
        {
            if (!string.IsNullOrEmpty(basket.GetProperty("PdkParcelShopId", string.Empty)))
            {
                return string.Format("{0}: {1} ({2})", base.GetSummeryString(model, basket, deliveryType), basket.GetProperty("PdkParcelShopName", string.Empty), basket.GetProperty("PdkParcelShopId", string.Empty));
            }
            else
            {
        ...
        public override string GetOrderString(CatalogModel model, Order order, DeliveryType deliveryType)
        {
            ...
            {
                return string.Format("{0}: {1} ({2})", base.GetOrderString(model, order, deliveryType), order.GetProperty("PdkParcelShopName", string.Empty), order.GetProperty("PdkParcelShopId", string.Empty));
            }
            else
            {
