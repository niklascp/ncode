...
namespace nCode.JobScheduling
{
    ...
    {
        public static string Queue(this JobEngine jobEngine, Type backgroundJobType)
        {
            ...
            var method = jobEngine.GetType().GetMethod("Queue", new Type[] { });
            var genericMethod = method.MakeGenericMethod(backgroundJobType);
            return (string)genericMethod.Invoke(jobEngine, null);
        }
    }
}
